...
static int show_menu = 0, menu_start = 0, menu_size;
...
int ui_scroll_menu(int dir) {
    int proceed = 0;
    switch (dir)
    {
	case -1:
	    if (menu_start > 0) proceed = 1;
	    break;
	case 1:
	    if (menu_size > menu_start+text_rows-8-menu_top) proceed = 1;
	    break;
    }
    if (proceed) {
	int x = 0;
